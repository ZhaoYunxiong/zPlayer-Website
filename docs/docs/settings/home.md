...
<img class="zplayer-screenshot" src="../../assets/screenshots/settings-home.webp" width="1727" height="1268" loading="lazy" decoding="async" alt="zPlayer 首页设置页面，展示栏目、推荐数量和联网搜索选项" />
<p class="zplayer-screenshot-caption">图：首页设置集中控制栏目、推荐数量、联网搜索和剧集详情显示方式。</p>

::: tip 只想播放一个文件？
使用左上角的“打开 → 文件”会更快；设置好 Windows 默认应用后也可从资源管理器双击。首页设置主要服务于长期维护的媒体库，详细流程见[首页管理](/docs/media/home-management)。
:::
...
如果你经常打开软件就想看到刚下载的内容，可以缩短列表缓存更新间隔；“最近更新天数”只决定多早以前的文件仍算最近更新，不会让扫描更频繁。首页请求太多、启动较慢时，可以适当增大缓存间隔。
...
开启后可能会增加首次处理时间、磁盘占用和 AI 调用费用。没有配置嵌入模型时，这些开关不会凭空产生搜索能力；先看[AI 模型](/docs/settings/ai)，确认模型和数据发送范围。
