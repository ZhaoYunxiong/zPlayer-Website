# 历史记录：让 zPlayer 记住你看到哪里

历史记录不是简单的“播放过的文件列表”。它会把最近播放、当前进度、已观看状态和继续观看串起来，让你下次打开 zPlayer 时能直接回到上次停下的地方。
...
开始播放后，zPlayer 会在播放过程中更新本地进度；暂停、停止、切换片源或关闭播放器时，会尽量保存最后位置。播放记录保存在本地，不需要登录任何外部账户也能使用。

历史记录可以帮助你：

- 从“历史记录”入口找回最近播放过的内容；
- 从首页“继续观看”直接回到未完成的电影、剧集或单集；
- 让动态栏目按观看状态、最近播放或更新时间自动筛选；
- 在媒体详情中确认当前作品、季集和片源是否对应正确。
