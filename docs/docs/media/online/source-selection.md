...
在线媒体最常用的主线是：**普通浏览站点或搜索 → 点击结果 → 按设置进入媒体详情或直接选择线路 → 播放**。

“设置 → 在线库 → 优先打开媒体详情”默认开启。开启时会先尝试 TMDB 详情，关闭时直接进入站点线路与剧集选择。下面按实际点击顺序说明两条路径。
...
点击作品卡片后，zPlayer 读取“优先打开媒体详情”设置。这个设置只决定先显示哪种界面，不会更换站点或把在线内容加入本地媒体库。

### 开启：优先使用统一媒体详情

zPlayer 会先读取播放列表并尝试从 TMDB 匹配作品。匹配成功后进入统一媒体详情页，你可以先确认资料，再选择季、集和在线片源。

### TMDB 找不到匹配：使用简易弹窗兜底

如果站点没有返回线路，或标题、类型等信息无法建立媒体详情，zPlayer 不会因为详情失败而阻止播放，而是回落到线路与集数选择。选择后即可进入播放器。

<img class="zplayer-screenshot" src="../../../assets/screenshots/online-source-selection.webp" width="1727" height="1268" loading="lazy" decoding="async" alt="zPlayer 在线媒体无法匹配 TMDB 时的简易线路与集数选择弹窗" />
<p class="zplayer-screenshot-caption">图：TMDB 找不到匹配时，在线媒体使用简易弹窗选择线路和集数作为播放兜底。</p>

### 关闭：直接选择线路和剧集

关闭“优先打开媒体详情”后，点击卡片会跳过 TMDB 详情匹配，直接加载站点提供的线路和剧集。适合站点标题较特殊、TMDB 匹配等待较长，或你只想少一步操作的场景。
...
首页右上角“更多 → 联网推荐”也能把推荐作品交给在线媒体来源尝试匹配片源，具体见[联网推荐](/docs/media/home/search-and-recommendation#online-recommendation)。
